...
class App extends Component {
	...
	*/
	...
	createtokenandDAO = (ipfsID) => {
		...
		let ipfsURL;
		if (app.state.ipfsID) {
			ipfsURL = 'https://ipfs.mecasso.live/ipfs/' + encodeURIComponent(ipfsID);
		}
		else {
	...
	addDAOtoDB = (contractAddress) => {
		...
		var ipfsURL = 'https://ipfs.mecasso.live/ipfs/' + encodeURIComponent(app.state.ipfsID);
		...
		let userID 
		if(app.state.socialIDs.Youtube){
			userID = app.state.socialIDs.Youtube.email
		} else {
			userID = app.state.fields.creatorname + '-instagram@gmail.com'
		}
		...
		fetch(url, {
			...
			if (response) {
				...
				app.setDAOlist();
				app.setNav('2d');
				if(app.state.socialIDs.Instagram){
					app.migrateInstagramPosts(app.state.socialIDs.Instagram.media)
				}
	...
	migrateInstagramPosts = (mediaData) => {
		let app = this;
		...
		var params = JSON.stringify({
			...
			publisheddatetime: mediaData.timestamp
		...
		fetch(url, {
			...
			if (response) {
				console.log("Instagram Post added to MongoDB ", response);
			}
			else {
				console.log(error);
			}
	...
	setSocialCard = (social, channelData) => {
		...
		if (social == 'Instagram') {
			...
			socialIDs.Instagram = channelData;

			FieldList.Instagram = 'YesAuth';
		...
		if (social == 'Facebook') {
			...
			socialIDs.Facebook = channelData;

			FieldList.Facebook = 'YesAuth';
	...
	*/
						...
						contract.methods
							...
							.call({ from: app.web3.eth.defaultAccount }).then(function (response, err) {
								...
								if (response) {
									...
									var url = 'https://ipfs.mecasso.live/ipfs' + tokenURI;
	...
	increaseTokenCap = () => {
		...
		contract.methods
			.increaseCapProposal(app.state.fields.description,
				...
				parseInt(app.state.fields.debateperiod) * 24 * 60)
			.send({ from: app.state.account }).then(function (response, err) {
				if (response) {
					console.log('response', response)
					app.setState({
	...
	newProposal = () => {
		...
		console.log('all params', app.state.fields.recipient, app.state.fields.amount, app.state.fields.heading, app.state.fields.description, app.state.fields.debateperiod, app.state.account)

		contract.methods
			.newProposal(app.state.fields.recipient,
				Web3.utils.toWei(app.state.fields.amount),
			...
			.send({ from: app.state.account }).then(function (response, err) {
				if (response) {
					console.log(response)
					let ProposalID = response.events.ProposalAdded.returnValues.proposalID;
					app.setState({
						ProposalID: ProposalID,
						txHash: response.transactionHash
					})
	...
	sendToken = () => {
		...
		console.log('sendtoken params', app.state.fields.sendAddress, Web3.utils.toWei(app.state.fields.currentTxnAmount), app.state.account)

		contract.methods
			.transfer(app.state.fields.sendAddress, Web3.utils.toWei(app.state.fields.currentTxnAmount))
			.send({ from: app.state.account }).then(function (response, err) {
				if (response) {
					console.log(response)
					app.setState({
	...
	ipfsNFT = () => {
		...
		let ipfsWriteUrl = 'https://ipfswrite.mecasso.live/api/v0/add'

		console.log('Calling IPFS Write @', ipfsWriteUrl)
		fetch(ipfsWriteUrl, { method: 'POST', body: filedata, mode: 'cors' }).
			then(function (response, error) {
   ...
   */
	...
	setField = (value) => {
		...
		if (value == 1) {
			if (FieldList.Youtube !== 'YesAuth') { FieldList.Youtube = 'Yes' }
			if (FieldList.Instagram === 'Yes') { FieldList.Instagram = '' }
			if (FieldList.Facebook === 'Yes') { FieldList.Facebook = '' }
			if (FieldList.Linktree === 'Yes') { FieldList.Linktree = '' }
			this.setState({
				FieldList: FieldList
			});
		}
		...
		if (value == 2) {
			if (FieldList.Instagram !== 'YesAuth') { FieldList.Instagram = 'Yes' }
			if (FieldList.Youtube === 'Yes') { FieldList.Youtube = '' }
			if (FieldList.Facebook === 'Yes') { FieldList.Facebook = '' }
			if (FieldList.Linktree === 'Yes') { FieldList.Linktree = '' }
			this.setState({
				FieldList: FieldList
		...
		if (value == 3) {
			if (FieldList.Facebook !== 'YesAuth') { FieldList.Facebook = 'Yes' }
			if (FieldList.Youtube === 'Yes') { FieldList.Youtube = '' }
			if (FieldList.Instagram === 'Yes') { FieldList.Instagram = '' }
			if (FieldList.Linktree === 'Yes') { FieldList.Linktree = '' }
			this.setState({
		...
		if (value == 4) {
			if (FieldList.Linktree !== 'YesAuth') { FieldList.Linktree = 'Yes' }
			if (FieldList.Youtube === 'Yes') { FieldList.Youtube = '' }
			if (FieldList.Instagram === 'Yes') { FieldList.Instagram = '' }
			if (FieldList.Facebook === 'Yes') { FieldList.Facebook = '' }
			this.setState({
		...
		return (
			...
			<div className="bg-img">
				...
				<Container onInputChangeUpdateField={this.onInputChangeUpdateField}
					...
					migrateInstagramPosts={this.migrateInstagramPosts}
				/>
			</div>
		)
